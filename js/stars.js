...
export function createStars(scene, count = 3000) {
 ...
 function morphStarsToPoints(indices, worldPoints, { duration = 10, ease = 'power2.inOut', onUpdate, onComplete } = {}) {
  ...
  indices.forEach((idx, i) => {
    ...
    const local = stars.worldToLocal(p.clone()); // ← 追加
    ...
    return gsap.to(state, {
      ...
      onComplete: () => {
        ...
        // stars.js の onComplete 内、焼き込み直後
const sample = stars.localToWorld(new THREE.Vector3(
  posArr[indices[0]*3], posArr[indices[0]*3+1], posArr[indices[0]*3+2]
));
console.log('star sample vs expected', {
  sample: sample.toArray(),
  // ここにequationAssembly.js側から渡ってきたpsiBaseの値を貼り付けて比較
  distanceFromPsiBase: sample.distanceTo(new THREE.Vector3(/* psiBaseの値をここに */)),
});
        console.log('star world pos sample', stars.localToWorld(new THREE.Vector3(
  posArr[indices[0]*3], posArr[indices[0]*3+1], posArr[indices[0]*3+2]
)));
