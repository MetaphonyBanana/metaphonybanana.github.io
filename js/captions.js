...
function showTopPhrase(caption, text, durationSeconds) {
  if (!text) return;
  caption.setPosition(TOP_PHRASE_POSITION.leftPercent, TOP_PHRASE_POSITION.topPercent);
  caption.setText(text);
  setTimeout(() => caption.hide(), durationSeconds * 1000);
}
...
export function showEquationCompletePhrase(caption) {
  showTopPhrase(caption, EQUATION_COMPLETE_PHRASE, EQUATION_COMPLETE_PHRASE_DURATION);
}
...
export function showWearItAWhilePhrase(caption) {
  showTopPhrase(caption, WEAR_IT_AWHILE_PHRASE, WEAR_IT_AWHILE_DURATION);
}