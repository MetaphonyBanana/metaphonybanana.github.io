...
export const STAR_CONVERGENCE = {
  ...
  //   最後まで文字として読めるようにする)。
  psiFinal: { count: 'all', duration: 30 },
};

// onComplete(indices, targetWorld): 収束が完了して焼き込みまで終わった対象の星indices配列と、
// その収束先の基準座標(ワールド座標)を渡す。
//
// ⚠ 経緯: 以前は「実際に生成したworldPointsの重心」をPNGの配置に使っていたが、
// ψのような左右非対称な字形だと、インク(不透明ピクセル)の分布の重心は
// 図形としての基準位置(=symbolLocalTargetPositionが返す、他のシンボルと同じ数式内の
// 正しい配置場所)とは一致しない。PNGスプライトの位置(symbolLocalTargetPosition)は
// 数式全体のレイアウトとして正しいことが確認できているので、逆に「点群の方を
// その正しい基準位置にきっちり合わせる」= 生成したworldPoints全体を、
// (基準位置 − 生成した点群の重心)ぶん平行移動して、字形はそのままに重心だけ
// 基準位置へ一致させる。こうすれば、収束後の星の重心とPNGの表示位置は
// (定義上)常に完全一致する。
function convergeStarsToSymbol(symbolKey, { starField, camera, vertexLocal, onComplete }) {
  ...
  // これが「正しい」基準位置(=PNGスプライトが最終的に置かれる場所そのもの)。
  const targetWorld = camera.localToWorld(basePointLocal.clone());
  ...
  computeGlyphWorldPoints(symbolKey, { camera, basePointLocal, count: indices.length }).then((worldPoints) => {
    ...
    // 生成した点群(字形)の重心を求め、それがtargetWorldに来るように点群全体を平行移動する。
    // これで字形の相対的な形は保ったまま、重心だけ「正しい位置」に強制一致させられる。
    const rawCentroid = new THREE.Vector3();
    worldPoints.forEach((p) => rawCentroid.add(p));
    rawCentroid.divideScalar(worldPoints.length);
    const correction = targetWorld.clone().sub(rawCentroid);
    const centeredPoints = worldPoints.map((p) => p.clone().add(correction));

    starField.morphStarsToPoints(indices, centeredPoints, {
      duration,
      onComplete: () => { if (onComplete) onComplete(indices, targetWorld); },
    });
  });
}
...
//
// centroidWorld → targetWorld: convergeStarsToSymbol側で使った「正しい基準位置」
//   (=数式レイアウト上のψの位置。星の点群はここへ重心が来るよう補正済みなので、
//   星の収束先とPNGの表示位置は理論上ここで完全一致する)。psiSpriteはcameraの子
//   (ローカル座標系)なので、camera.worldToLocal()でその時点のカメラ変換に合わせて
//   ローカル座標へ変換し直す。
const PSI_PNG_CROSSFADE_DURATION = 3;

function crossfadeStarsToPsiPng({ assembly, starField, camera, psiIndices, targetWorld, vertexLocal, onComplete }) {
  ...
  const targetLocal = targetWorld
    ? camera.worldToLocal(targetWorld.clone())
    : symbolLocalTargetPosition('psiFinal', vertexLocal); // フォールバック(基本的には通らない)
  psiSprite.position.copy(targetLocal);
  psiSprite.visible = true;
  psiSprite.material.opacity = 0;
  gsap.to(psiSprite.material, {
    opacity: 1,
    duration: PSI_PNG_CROSSFADE_DURATION,
    ease: 'power1.inOut',
  });
  starField.fadeGlyph(PSI_PNG_CROSSFADE_DURATION, onComplete);
}
          ...
          gsap.to(hamiltonianSprite.position, {
            ...
            onComplete: () => {
              ...
              convergeStarsToSymbol('psiFinal', {
                ...
                onComplete: (psiIndices, targetWorld) => {
                  // 仕上げ: ψを形作っていた星の集合を、本物のpsi.pngスプライトへ
                  // クロスフェードして置き換える(星側はここでフェードアウトして消える)。
                  // targetWorldを渡すことで、星の収束先(補正済み)とPNGの表示位置を一致させる。
                  crossfadeStarsToPsiPng({
                    assembly, starField, camera, psiIndices, targetWorld, vertexLocal,
                    onComplete: () => {
